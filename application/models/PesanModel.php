...
class PesanModel extends CI_Model
{
	...
	public function simpan_print($data){
		$this->db->insert('sipesan_print',$data);
		return $this->db->affected_rows();
	}
	public function lihat_print_by_id($id){
		$this->db->where('print_id',$id);
		return $this->db->get('sipesan_print')->row_array();
	}
