...
class BayarController extends CI_Controller{
	...
	public function keranjang(){
		...
		$data = array(
			...
			'print' => $this->BayarModel->lihat_keranjang_print($this->session->userdata('session_id'),'belum',$keranjang['keranjang_id'])->result_array(),
			'title' => 'Keranjang | VirtualPrints'
		);
	...
	public function bayar($id){
		...
		$data = array(
			...
			'print' => $this->BayarModel->lihat_keranjang_print($this->session->userdata('session_id'),'belum',$pesanan['keranjang_id'])->result_array(),
			'title' => 'Pembayaran | VirtualPrints'
		);
		$this->load->view('frontend/templates/header',$data);
	...
	public function selesai($bank,$id){
		$dataBank = array(
			'bni' => 'Bank BNI 123456789 Atas Nama VirtualPrints ',
			'bri' => 'Bank BRI 115511026 Atas Nama VirtualPrints '
		);
		$data = array(
			'bank' => $dataBank[$bank],
			'pesanan' => $this->BayarModel->lihat_keranjang_status_selesai($this->session->userdata('session_id'),'bayar_menunggu',$id)->row_array(),
			'title' => 'Terima Kasih | VirtualPrints'
		);
		$this->load->view('frontend/templates/header',$data);
	...
	public function konfirmasi($id){
		if (isset($_POST['konfirmasi'])){
			...
			$data = array(
				'title' => 'Konfirmasi Pembayaran | VirtualPrints',
