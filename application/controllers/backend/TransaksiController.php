...
//		var_dump($konfirmasi);die;
		$data = array(
			...
			'print' => $this->BayarModel->lihat_keranjang_print($transaksi['keranjang_pengguna_id'], 'bayar_menunggu', $transaksi['keranjang_id'])->result_array(),
		);
		$this->load->view('backend/templates/header');
	...
	public function email($id)
	{
		...
		$config['smtp_user'] = "webnyawiji@gmail.com";
		$config['smtp_pass'] = "ufzqizjeyoskvznw";
		$config['charset'] = "utf-8";
		$config['mailtype'] = "html";
		$config['newline'] = "\r\n";

		$this->email->initialize($config);
		$this->email->from('webnyawiji@gmail.com', 'nyawiji web');
		$list = array($transaksi['pengguna_email']);
		$this->email->to($list);
		$this->email->subject('Pesanan Selesai');
		$message = $transaksi['pengguna_nama'] . ', Pesanan anda sudah selesai, silahkan datang ke alamat kami untuk menjemput pesanan anda (Jalan Banyu Urip Wetan 2/123 RT 06 RW 06, Kota SBY, Jawa Timur 60254)';
		...
		if ($data['print'] != null) {
			...
			foreach ($data['print'] as $key => $value) {
				$harga = $harga + $value['print_total'];
			}
			$print = array(
				'jenis' => 'Print',
				'jumlah' => count($data['print']),
				'harga' => $harga
			);
			array_push($table, $print);
		}
