...
class PesanController extends CI_Controller
{
	...
	public function pesanSpanduk()
	{
		if (isset($_POST['keranjang'])) {
			...
			if ($bahan == 'biasa') {
				...
				$total =  $jumlah * 500 + 8000;
			} elseif ($bahan == 'bagus') {
				$total =  $jumlah * 500 + 30000;
			}

			$config['upload_path'] = './assets/images/spanduk/';
			$config['allowed_types'] = 'xlsx|docx|pdf|jpg|png|jpeg';
		...
		$data = array(
			'title' => 'Pesan Spanduk | VirtualPrints'
		);
	...
	public function pesanStiker(){
		...
		$data = array(
			'title' => 'Pesan Stiker | VirtualPrints'
		);
		$this->load->view('frontend/templates/header',$data);
	...
	public function pesanKartu(){
		...
		$data = array(
			'title' => 'Pesan Kartu Nama | VirtualPrints'
		);
		$this->load->view('frontend/templates/header',$data);
	...
	public function pesanBrosur(){
		...
		$data = array(
			'title' => 'Pesan Brosur | VirtualPrints'
		);
		$this->load->view('frontend/templates/header',$data);
	...
	public function pesanPrint(){
		if (isset($_POST['keranjang'])) {
			$printId = 'PRT-' . substr(time(), 5);
			$ukuran = $this->input->post('ukuran');
			$bahan = $this->input->post('bahan');
			$jumlah = $this->input->post('jumlah');
			$estimasi = $this->input->post('estimasi');
			...
			$config['upload_path'] = './assets/images/print/';
			$config['allowed_types'] = 'xlsx|docx|pdf|jpg|png|jpeg';
			$this->load->library('upload', $config);
			$this->upload->initialize($config);
			...
			if (!$this->upload->do_upload('upload')) {
				...
				if ($allCart == null){
					$cartId = 'CRT-' . substr(time(), 5);
					$dataPrint['print_keranjang_id'] = $cartId;
					$dataCart = array(
						'keranjang_id' => $cartId,
						'keranjang_pengguna_id' => $this->session->userdata('session_id'),
						'keranjang_total' => $total,
					);
					$this->PesanModel->simpan_print($dataPrint);
					$this->BayarModel->simpan_keranjang($dataCart);
					$this->session->set_flashdata('alert', 'pesan_sukses');
					redirect('print');
				} else {
		...
		$data = array(
			'title' => 'Pesan Cetak | VirtualPrints'
		);
		$this->load->view('frontend/templates/header',$data);
		$this->load->view('frontend/pesanan/print');
		$this->load->view('frontend/templates/footer');
	}
	...
	public function hapusPrint($id){
		$brosur = $this->PesanModel->lihat_brosur_by_id($id);
		$keranjang_id = $brosur['print_keranjang_id'];
		$keranjang = $this->BayarModel->lihat_keranjang_by_id($keranjang_id);
		$total = $brosur['print_total'];
		$data = array(
			'keranjang_total' => $keranjang['keranjang_total'] - $total
		);
		$this->PesanModel->delete('print_id',$id,'sipesan_print');
		$this->BayarModel->update_keranjang($keranjang_id,$data);
		$this->session->set_flashdata('alert', 'pesan_hapus');
		redirect('keranjang');
	}
